...
'Provide an optional Macro to allow user to run the Stock Analysis Macro below on all sheets at once
    ...
    'Loop that runs our above StockAnalysis macro for every sheet
...
'select and apply "percentage" format to column (K), "Percent Change" - to match desired output
'select and apply "percentage" format to Q2 & Q3, "Greatest % Increase/Decrease" - to match desired output
...
'Set initial values
    ...
    'Setting initial stock's first daily volume to be the relative starting point for volumn aggregate result
